...
var BURP_PROXY_IP = "192.168.1.204";
var BURP_PROXY_PORT = "8083";
...
function getExportFunction(name, ret, args) {
    var funcPtr;
    // Use the modern, non-deprecated API 'findExportByName'
    // Passing 'null' as the first argument searches all loaded modules.
...
function parseElf(base) {
    //
    // MOVED FROM GLOBAL SCOPE TO HERE
    //
...
function parseMachO(base) {
    ...
    if (magic == 0xfeedfacf) {
        ...
        return; // Exit if magic is not for Mach-O 64-bit
    }
    ...
    for (var i = 0; i < cmdnum; i++) {
        ...
        if (cmd === 0x19) { // LC_SEGMENT_64
            ...
            var nsects = base.add(cmdoff + number_of_sections_offset).readU32(); // readU32 is correct here
            var secbase = base.add(cmdoff + section64_header_base_offset);

            if (cmdsize >= section64_header_base_offset + nsects * section64_header_size) {
                for (var j = 0; j < nsects; j++) {
                    var section_header = secbase.add(j * section64_header_size);
                    var secname = section_header.readUtf8String();
                    // In section_64, size is a uint64_t at offset 0x28
                    var section_size = section_header.add(0x28).readU64();
                    // In section_64, offset is a uint32_t at offset 0x30
                    var section_start_offset = section_header.add(0x30).readU32();

                    if (segname === '__TEXT' && secname === '__text') {
                        TEXT_segment_text_section_offset = section_start_offset;
                        TEXT_segment_text_section_size = section_size.toNumber();
                    } else if (segname === '__TEXT' && secname === '__cstring') {
                        TEXT_segment_cstring_section_offset = section_start_offset;
                        TEXT_segment_cstring_section_size = section_size.toNumber();
                    } else if ((segname === '__DATA' || segname === '__DATA_CONST') && secname === '__const') {
                        DATA_segment_const_section_offset = section_start_offset;
                        DATA_segment_const_section_size = section_size.toNumber();
                    }
                }
            }
        }
    ...
    function init(base) {
        ...
        var int_getSockAddr = setInterval(() => {
            if (GetSockAddr_func_addr != null) {
                // **** FIX: Wrap in a try...catch to ensure the interval is always cleared ****
                try {
                    console.log("[*] Hook GetSockAddr function");
                    hook("GetSockAddr");
                } catch (e) {
                    console.error("Error in GetSockAddr hook:", e.stack);
                } finally {
                    // This will now run even if the hook() call throws an error
                    clearInterval(int_getSockAddr);
                }
            }
        }, 100); // Using a small delay is better practice than 0
